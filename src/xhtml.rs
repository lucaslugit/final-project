...
//this was old version of part1. because of some miscommunication, part4 was based on this version of code instead of new version of code. Just to explain it if it looks weird.
//We still made it :)
...
struct MyScanner {
    ...
    token_vec:Vec<MyToken>,
    current_token_index:i32,
    
}
impl MyScanner{
    fn get_next_token(&mut self)->&MyToken {
...
fn init_token(length:usize ,all_tokens:Vec<String>)->Vec<MyToken>{
    let mut all_token : Vec<MyToken> = Vec::new();
    for i in 0..length{
        let tem_token = MyToken{
